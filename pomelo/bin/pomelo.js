 ...
 */
var Pomelo = (function (ByteArray) {


  var CustomSokcet = (function () {
    if (egret.Capabilities.runtimeType == egret.RuntimeType.NATIVE) {

      return __global["egret_native"]["WebSocket"];
    }
    else {
  ...
  pomelo.init = function (params, wss, cb, errorCb, closeCb, thisArgs) {
  ...
  pomelo.initWebSocket = function (url, cb) {
    ...
    var onmessage1 = function (msg) {
      self.processPackage(Package.decode(msg), cb);
      // new package arrived, update the heartbeat timeout
      if (self.heartbeatTimeout) {
        self.nextHeartbeatTimeout = Date.now() + self.heartbeatTimeout;
      }
    ...
    var onerror = function (event) {
      ...
      if (self.errorCallback) {
        self.errorCallback.apply(self.thisArgs, event);
      }
    ...
    var onclose = function (event) {
      ...
      if (self.closeCallBack) {
        self.closeCallBack.apply(self.thisArgs, event);
      }

    };

    this.socket = new CustomSokcet(url);

    if (egret.Capabilities.runtimeType == egret.RuntimeType.NATIVE) {

      this.socket.onOpen = onopen;
      this.socket.onMessage = onmessage1;
  ...
  pomelo.handshake = function (data) {
    ...
    if (this.initCallback) {
      this.initCallback.apply(this.thisArgs, data);
      this.initCallback = null;
    }
  };
  ...
  pomelo.processMessage = function (pomelo, msg) {
    if (!msg.id) {
      ...
      this.emit('server_push_message', msg);
      return;
    }
